...
async def test_cogwit_cognify_success():
    ...
    mock_send_api_request.return_value = SuccessResponse(status=200, data={"pipeline_run_id": str(pipeline_run_id)})
    ...
    with patch("cogwit_sdk.cogwit.cogwit.send_api_request", mock_send_api_request):
        ...
        assert mock_send_api_request.call_args[0] == ("/cognify", "post", {"X-Api-Key": dummy_api_key}, {"dataset_ids": dummy_dataset_ids})
...
async def test_cogwit_cognify_assert_api_request_args():
    ...
    mock_send_api_request = AsyncMock()
    mock_send_api_request.return_value = DummyResponse(status=400, error={"error": "Invalid dataset IDs"})
    with patch("cogwit_sdk.cogwit.cogwit.send_api_request", mock_send_api_request):
...
async def test_cogwit_cognify_failure():
    ...
    mock_send_api_request = AsyncMock()
    mock_send_api_request.return_value = ErrorResponse(status=400, error={"error": "Dataset not found"})
    with patch("cogwit_sdk.cogwit.cogwit.send_api_request", mock_send_api_request):
        dummy_dataset_ids = [UUID("12345678-1234-1234-1234-123456789abc")]
        result = await cogwit_instance.cognify(dataset_ids=dummy_dataset_ids)
...
async def test_cogwit_cognify_any_non_success_response_returns_CognifyError():
    ...
    mock_send_api_request.return_value = DummyResponse(status=500, error={"error": "Internal server error"})
    with patch("cogwit_sdk.cogwit.cogwit.send_api_request", mock_send_api_request):
        dummy_dataset_ids = [
            UUID("12345678-1234-1234-1234-123456789abc"),
        ...
        result = await cogwit_instance.cognify(dataset_ids=dummy_dataset_ids)
        assert result == CognifyError(status=500, error={"error": "Internal server error"})
        assert mock_send_api_request.call_count == 1
...
async def test_cogwit_cognify_with_empty_dataset_ids():
    ...
    with patch("cogwit_sdk.cogwit.cogwit.send_api_request", mock_send_api_request):
        ...
        assert mock_send_api_request.call_args[0] == ("/cognify", "post", {"X-Api-Key": dummy_api_key}, {"dataset_ids": empty_dataset_ids})
        assert result == CognifyResponse(status=200, pipeline_run_id=pipeline_run_id)
        assert mock_send_api_request.call_count == 1