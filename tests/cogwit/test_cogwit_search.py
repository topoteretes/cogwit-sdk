...
async def test_cogwit_search_different_search_types():
    ...
    for search_type, expected_payload in test_cases:
        mock_send_api_request = AsyncMock()
        mock_send_api_request.return_value = SuccessResponse(status=200, data=expected_payload)
...
async def test_cogwit_search_assert_api_request_args():
    ...
    mock_send_api_request = AsyncMock()
    mock_send_api_request.return_value = DummyResponse(status=400, error={"error": "Invalid query"})
    with patch("cogwit_sdk.cogwit.cogwit.send_api_request", mock_send_api_request):
        dummy_query_text = "Invalid query"
        dummy_query_type = SearchType.RAG_COMPLETION
        await cogwit_instance.search(query_text=dummy_query_text, query_type=dummy_query_type)
        assert mock_send_api_request.call_count == 1
        assert mock_send_api_request.call_args[0] == (
            "/search", 
            "post", 
...
async def test_cogwit_search_failure():
    ...
    mock_send_api_request = AsyncMock()
    mock_send_api_request.return_value = ErrorResponse(status=404, error={"error": "No results found"})
    with patch("cogwit_sdk.cogwit.cogwit.send_api_request", mock_send_api_request):
        dummy_query_text = "Nonexistent data"
        dummy_query_type = SearchType.CHUNKS
        result = await cogwit_instance.search(query_text=dummy_query_text, query_type=dummy_query_type)
...
async def test_cogwit_search_any_non_success_response_returns_SearchError():
    ...
    mock_send_api_request.return_value = DummyResponse(status=500, error={"error": "Search service unavailable"})
    with patch("cogwit_sdk.cogwit.cogwit.send_api_request", mock_send_api_request):
        ...
        assert result == SearchError(status=500, error={"error": "Search service unavailable"})
        assert mock_send_api_request.call_count == 1
